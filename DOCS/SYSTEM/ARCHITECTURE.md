...
| 화면 크기 | 844 × 390 px (가로 모드 고정) |
| 화면 방향 | `orientation: LANDSCAPE` — 세로 모드로 실행하면 회전 안내 표시 |
...
4. entities/
   ...
   ├── AllyUnit.js
   ├── Paladin.js      AllyUnit 상속 — AllyUnit 이후, Hero 이전
   ├── EnemyUnit.js
...
| `window.AllyUnit` | Class | 아군 유닛 (범위 공격 포함) |
| `window.Paladin` | Class | 성기사 유닛 (AllyUnit 상속, 오라 버프) |
| `window.EnemyUnit` | Class | 적군 유닛 |
| `window.Hero` | Class | 영웅 유닛 (AllyUnit 상속, 스킬 보유) |
...
└── Unit (Unit.js)
    ├── AllyUnit (AllyUnit.js)         — 아군 유닛 기반. splashRadius 범위 공격 지원
    │   ├── Paladin (Paladin.js)       — 성기사. 오라 버프 적용 [PROTO BEGIN/END 격리]
    │   └── Hero (Hero.js)             — 영웅. 스킬(광역 3배) + 불화살 ArcProjectile 사용
    └── EnemyUnit (EnemyUnit.js)       — 적군 유닛. isElite 플래그로 정예병 구분
```

`Paladin`은 `[PROTO BEGIN] ~ [PROTO END]` 주석으로 코드가 격리되어 있다. 프로토타입 코드 제거 가이드는 `DOCS/FEATURES/PROTO_GUIDE.md` 참조.

---

## 전투 좌표 체계

가로 모드로 전환되면서 전체 전투 공간이 수평으로 확장되었다. HUD는 화면 하단에 고정 배치되며, 전투 영역과 완전히 분리된다.
...
- `GAME_WIDTH` = 844, `GAME_HEIGHT` = 390
- `BATTLE_Y` = 250 — 유닛 발이 닿는 지면 y 좌표
- `HUD_HEIGHT` = 110 — 화면 하단 HUD 영역 높이
- 아군 유닛은 `ALLY_CASTLE_X + 30` 에서 소환되어 오른쪽(+x)으로 이동
- 적군 유닛은 `ENEMY_CASTLE_X - 30` 에서 소환되어 왼쪽(-x)으로 이동
